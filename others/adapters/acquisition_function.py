...
class AEI(EI):
    def __init__(self, xi: float = 0.0) -> None:
        super(AEI, self).__init__(xi=xi)
        logging.info("!!!Define AEI!!!")
    @property
    def name(self) -> str:
        return "Augmented Expected Improvement"
    
    def _compute(self, X: np.ndarray) -> np.ndarray:
        """Compute AEI acquisition value
        ...
        if not self._log:
            ...
            ## ADD TERMS ####################################################
            noise_level = self._model._gp.alpha
            f = f * (1 - (noise_level / np.sqrt(v + noise_level**2)))
            #################################################################
